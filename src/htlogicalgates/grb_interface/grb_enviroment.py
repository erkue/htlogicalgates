...
class Enviroment:
    ...
    def create_bin_var(self) -> gp.Var:
        self._bin_counter += 1
        v = self.model.addVar(vtype=GRB.BINARY, name=Enviroment.NAME_BIN + str(self._bin_counter))
        return v
    ...
    def create_predef_var(self) -> gp.Var:
        ...
        v = self.model.addVar(vtype=GRB.BINARY, name=Enviroment.NAME_CONST + str(self._predef_counter))
        return v
    ...
    def set_target_function(self, expr : Expression):
        self.model.setObjective(sum([a*self._add_AND_constraint(b) for a,b in zip(expr._prefac, expr._nums)], start=expr._const), GRB.MINIMIZE)
    ...
    def _add_AND_constraint(self, vars : List[gp.Var]) -> gp.Var:
        ...
        self.model.addGenConstrAnd(v, vars, name=Enviroment.NAME_AND + str(self._bin_counter))
        return v
